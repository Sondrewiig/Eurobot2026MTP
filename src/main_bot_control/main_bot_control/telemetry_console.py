...
class TelemetryConsole(Node):
    def __init__(self):
        ...
        self.enemy_overhead_pose = None
        self.enemy_ground_truth_pose = None
        self.fused_pose = None              # final combined estimate
        self.localization_status = None
        self.selected_tag = None

        # Strategy stopwatch shown in the telemetry GUI.
        # In sim.launch.py this node gets use_sim_time=True, so this follows
        # Gazebo /clock instead of wall-clock/IRL time.
        self.strategy_timer_name = None
        self.strategy_timer_start_s = None
        self.strategy_timer_stop_s = None
        self.strategy_timer_elapsed_s = None
        self.strategy_timer_reported_elapsed_s = None
        self.strategy_timer_running = False
        self.strategy_timer_status = "IDLE"
        ...
        self.opencr_avoidance_status = None
        ...
        self.create_subscription(Pose2D, "/bot_pose_ground_truth", self.ground_truth_cb, 10)
        self.create_subscription(Pose2D, "/vision/enemy_pose", self.enemy_overhead_pose_cb, 10)
        self.create_subscription(Pose2D, "/enemy_pose_ground_truth", self.enemy_ground_truth_cb, 10)
        ...
        self.create_subscription(Image, "/vision/board_debug", self.overhead_debug_image_cb, qos_profile_sensor_data)
        ...
        self.create_subscription(String, "/opencr/avoidance_status", self.opencr_avoidance_status_cb, 10)
        ...
        self.go_zone_pub = self.create_publisher(Int32, "/opencr/cmd/go_zone", 10)
        self.go_pantry_pub = self.create_publisher(Int32, "/opencr/cmd/go_pantry", 10)
        self.mission_pub = self.create_publisher(String, "/opencr/cmd/mission", 10)
        ...
        self.strategy_timer_label = tk.Label(
            top_frame,
            text="Strategy: ---",
            font=("Arial", 16, "bold"),
            width=34,
        ...
        self.strategy_timer_label.pack(side="left", padx=(12, 0))
        ...
        # Overhead camera
        overhead_frame = tk.LabelFrame(image_row, text="Overhead Detection", padx=4, pady=4)
        overhead_frame.pack(side="left", fill="x", expand=True, padx=(4, 0))

        self.overhead_debug_title = tk.Label(
            overhead_frame,
            text="/vision/board_debug",
            font=("Arial", 10, "bold"),
        )
        self.overhead_debug_title.pack(anchor="w")

        self.overhead_debug_label = tk.Label(
            overhead_frame,
            text="No overhead debug image yet",
            bg="black",
            fg="white",
            anchor="center",
        )
        self.overhead_debug_label.pack(anchor="n", pady=(2, 0))

        # Keep references for container sizing
        self.left_debug_container = left_frame
        self.right_debug_container = right_frame
        self.overhead_debug_container = overhead_frame
    ...
    # ---------------- ROS/sim-time stopwatch helpers ----------------
    def _ros_time_s(self) -> float:
        """Current ROS time in seconds.

        In Gazebo simulation this is simulated match time when use_sim_time=True.
        On the real robot, where use_sim_time is normally false, this is normal
        wall-clock time.
        """
        return self.get_clock().now().nanoseconds * 1e-9

    def _format_duration_s(self, seconds: Optional[float]) -> str:
        if seconds is None:
            return "---"
    ...
    def _parse_reported_elapsed_s(self, text: str) -> Optional[float]:
        # Handles strings like: STRATEGY_COMPLETE strategy_1 elapsed_s=42.123
    ...
    def _strategy_timer_current_elapsed_s(self) -> Optional[float]:
        if self.strategy_timer_running and self.strategy_timer_start_s is not None:
            return max(0.0, self._ros_time_s() - self.strategy_timer_start_s)
        return self.strategy_timer_elapsed_s

    def _strategy_timer_text(self, compact: bool = False) -> str:
        elapsed = self._strategy_timer_current_elapsed_s()
        if elapsed is None:
            return "---"

        name = self.strategy_timer_name or "strategy"
        elapsed_txt = self._format_duration_s(elapsed)

        if self.strategy_timer_running:
            if compact:
                return f"{name} {elapsed:.1f}s SIM"
            return f"RUNNING {name}: {elapsed_txt} ({elapsed:.2f} s, SIM)"

        status = self.strategy_timer_status or "DONE"
        if compact:
            return f"{status} {elapsed:.1f}s SIM"

        txt = f"{status} {name}: {elapsed_txt} ({elapsed:.2f} s, SIM)"
        if self.strategy_timer_reported_elapsed_s is not None:
            txt += f"  controller={self.strategy_timer_reported_elapsed_s:.2f}s"
        return txt

    def _start_strategy_timer_from_event(self, text: str):
        parts = text.split()
        name = parts[1] if len(parts) >= 2 else "strategy"
        self.strategy_timer_name = name
        self.strategy_timer_start_s = self._ros_time_s()
        self.strategy_timer_stop_s = None
        self.strategy_timer_elapsed_s = None
        self.strategy_timer_reported_elapsed_s = None
        self.strategy_timer_running = True
        self.strategy_timer_status = "RUNNING"

    def _stop_strategy_timer_from_event(self, text: str, status: str):
        now_s = self._ros_time_s()
        self.strategy_timer_stop_s = now_s

        if self.strategy_timer_start_s is not None:
            self.strategy_timer_elapsed_s = max(0.0, now_s - self.strategy_timer_start_s)
        else:
            self.strategy_timer_elapsed_s = self._parse_reported_elapsed_s(text)

        self.strategy_timer_reported_elapsed_s = self._parse_reported_elapsed_s(text)
        self.strategy_timer_running = False
        self.strategy_timer_status = status

        parts = text.split()
        if len(parts) >= 2 and parts[0].startswith("STRATEGY_"):
            self.strategy_timer_name = parts[1]

    def _handle_strategy_timer_message(self, text: str, source: str = "event"):
        text = (text or "").strip()
        if not text:
            return

        if text.startswith("STRATEGY_START "):
            self._start_strategy_timer_from_event(text)
            return

        if text.startswith("STRATEGY_COMPLETE "):
            self._stop_strategy_timer_from_event(text, "DONE")
            return

        if self.strategy_timer_running and text.startswith("STRATEGY_") and source == "error":
            self._stop_strategy_timer_from_event(text, "ERROR")
            return

        if self.strategy_timer_running and text in ("STOPPED", "ESTOP"):
            self._stop_strategy_timer_from_event(text, text)
            return
    ...
    def ground_truth_cb(self, msg: Pose2D):
        self.ground_truth_pose = msg

    def enemy_overhead_pose_cb(self, msg: Pose2D):
        self.enemy_overhead_pose = msg

    def enemy_ground_truth_cb(self, msg: Pose2D):
        self.enemy_ground_truth_pose = msg
    ...
    def overhead_debug_image_cb(self, msg: Image):
        with self.latest_debug_lock:
            self.latest_overhead_debug_msg = msg
    ...
    def opencr_done_cb(self, msg: String):
        ...
        self._handle_strategy_timer_message(msg.data, source="done")
        self.command_feedback = f"DONE: {msg.data}"

    def opencr_error_cb(self, msg: String):
        self.opencr_error = msg.data
        self._handle_strategy_timer_message(msg.data, source="error")
        self.command_feedback = f"ERR: {msg.data}"

    def opencr_event_cb(self, msg: String):
        self.opencr_event = msg.data
        self._handle_strategy_timer_message(msg.data, source="event")
        self.command_feedback = f"EVENT: {msg.data}"

    def opencr_avoidance_status_cb(self, msg: String):
        self.opencr_avoidance_status = msg.data
    ...
    def fmt_avoidance_status(self):
        if self.opencr_avoidance_status is None:
            return "---"
        try:
            data = json.loads(self.opencr_avoidance_status)
        except Exception:
            return self.opencr_avoidance_status
        ...
        physical_blocked = bool(data.get("physical_blocked", blocked))
        goal_blocked = bool(data.get("goal_blocked", False))
        fresh = bool(data.get("enemy_pose_fresh", False))
        age = data.get("enemy_pose_age_s")
        pose = data.get("enemy_pose_mm")
        decision = data.get("decision_inside_perimeter", "stop")
        goal_decision = data.get("decision_goal_inside_perimeter", "hold")
        ...
        if physical_blocked:
            ...
            state = "clear"
        freshness = "fresh" if fresh else "stale/no enemy"
        prefix = f"{state} mode={decision} goal={goal_decision} action={action} ({freshness}, age={age}s)"
        if isinstance(pose, list) and len(pose) >= 3:
            return f"{prefix} enemy=({pose[0]:.0f},{pose[1]:.0f},{pose[2]:.0f}deg)"
        return prefix
    ...
    def build_telemetry_text_left(self):
        ...
        lines.append(f"Sim time:     {self._format_duration_s(self._ros_time_s())}")
        lines.append(f"Strategy t:   {self._strategy_timer_text(compact=False)}")
        ...
        lines.append(f"  Ground truth:     {self.fmt_pose(self.ground_truth_pose)}")
        lines.append(f"  Loc status:       {self.localization_status if self.localization_status is not None else '---'}")
        lines.append("")
        lines.append("Enemy bot:")
        lines.append(f"  Overhead estimate:{self.fmt_pose(self.enemy_overhead_pose)}")
        lines.append(f"  Ground truth:     {self.fmt_pose(self.enemy_ground_truth_pose)}")
        lines.append("")
    ...
    def build_telemetry_text_right(self):
        ...
        lines.append(f"  Avoidance:   {self.fmt_avoidance_status()}")
    ...
    def handle_command_text(self, cmd: str):
        ...
        if head == "zone":
            try:
                ...
                self.command_feedback = "ERROR: use zone <1-8>"
                return

            msg = Int32()
            msg.data = zone_id
            self.go_zone_pub.publish(msg)
            self.command_feedback = f"SENT: GO_ZONE {zone_id}"
            return
        ...
        if head == "pantry":
            try:
                ...
                self.command_feedback = "ERROR: use pantry <1-10>"
                return

            msg = Int32()
            msg.data = pantry_id
            self.go_pantry_pub.publish(msg)
            self.command_feedback = f"SENT: GO_PANTRY {pantry_id}"
            return

        # ---------------- Mission command ----------------
        if head == "mission":
            # Format: mission <zone> <pantry> [<zone> <pantry> ...]
            parts = tail.strip().split()
            if len(parts) < 2 or len(parts) % 2 != 0:
                self.command_feedback = "ERROR: use mission <zone> <pantry> [<zone> <pantry> ...]"
                return
            msg = String()
            msg.data = tail.strip()
            self.mission_pub.publish(msg)
            n_tasks = len(parts) // 2
            self.command_feedback = f"SENT: MISSION ({n_tasks} tasks)"
            return

        # ---------------- Strategy command ----------------
        if head == "strategy":
            if not tail.strip():
                self.command_feedback = "ERROR: use strategy <name-or-number>"
                return
            self.publish_raw_opencr_command(f"STRATEGY {tail.strip()}")
            self.command_feedback = f"SENT: STRATEGY {tail.strip()}"
            return
        ...
        if head == "go":
            ...
            tail_parts = tail.lower().split()
            if len(tail_parts) == 2 and tail_parts[0] == "zone":
                try:
                    zone_id = int(tail_parts[1])
                except Exception:
                    self.command_feedback = "ERROR: use go zone <1-8>"
                    return
            ...
            if len(tail_parts) == 2 and tail_parts[0] == "pantry":
                try:
                    pantry_id = int(tail_parts[1])
                except Exception:
                    self.command_feedback = "ERROR: use go pantry <1-10>"
                    return
        ...
        if head == "reload_config":
            self.publish_raw_opencr_command("RELOAD_CONFIG")
            self.command_feedback = "SENT: RELOAD_CONFIG"
            return
    ...
    def update_labels(self):
        ...
        self.strategy_timer_label.config(
            text=f"Strategy: {self._strategy_timer_text(compact=True)}"
        )
    ...
    def refresh_gui(self):
        ...
        overhead_msg_to_render: Optional[Image] = None
        ...
        with self.latest_debug_lock:
            ...
            if (
                self.latest_overhead_debug_msg is not None
                and now_ns - self.last_overhead_debug_render_ns >= self.debug_render_period_ns
            ):
                overhead_msg_to_render = self.latest_overhead_debug_msg

        # Calculate max image dimensions based on available space
        # Images span full width of main area, split in thirds
        row_w = max(240, self.image_row.winfo_width())
        root_h = max(400, self.root.winfo_height())
        max_w_per_image = max(120, (row_w // 3) - 20)
        ...
        if overhead_msg_to_render is not None:
            tk_img = self.rosimg_to_tk(overhead_msg_to_render, max_w_per_image, max_img_h)
            if tk_img is not None:
                self.overhead_debug_image_tk = tk_img
                self.overhead_debug_label.configure(image=self.overhead_debug_image_tk, text="")
                self.overhead_debug_label.image = self.overhead_debug_image_tk
            self.last_overhead_debug_render_ns = now_ns
